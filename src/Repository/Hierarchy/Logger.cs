...
namespace log4net.Repository.Hierarchy
{
	...
	public abstract class Logger : IAppenderAttachable, ILogger
	{
        ...
        public void Log(Type callerStackBoundaryDeclaringType, Level level, object message, string appenderName, Exception exception)
        {
            try
            {
                if (IsEnabledFor(level))
                {
                    ForcedLog((callerStackBoundaryDeclaringType != null) ? callerStackBoundaryDeclaringType : declaringType, level, message, appenderName, exception);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="appenderName"></param>
        /// <param name="exception"></param>
        virtual public void Log(Level level, object message, string appenderName, Exception exception)
        {
            if (IsEnabledFor(level))
            {
                ForcedLog(declaringType, level, message, appenderName, exception);
            }
        }
        ...
        /// <param name="appenderName"></param>
        /// <param name="exception"></param>
        virtual protected void ForcedLog(Type callerStackBoundaryDeclaringType, Level level, object message,string appenderName, Exception exception)
        {
            CallAppenders(new LoggingEvent(callerStackBoundaryDeclaringType, this.Hierarchy, this.Name, level, message,appenderName, exception));
        }
